...
async function run() {
    console.log("Discovering symbols for Phase 16 (15 min run)...");
    ...
    fs.writeFileSync(path.join(OUT_DIR, 'phase16_symbols_15m.json'), JSON.stringify(testPairs, null, 2));
    ...
    let msgs = 0;
    console.log("Starting 15-minute live polling...");
    ...
    while (Date.now() - start < DURATION) {
        ...
        msgs++;
        if (msgs % 30 === 0) console.log(`Time elapsed: ${Math.floor((Date.now()-start)/1000)}s`);
    }

    console.log("Finished 15-minute polling. Saving data...");
