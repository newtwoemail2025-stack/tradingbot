...
const DATA_DIR = path.join(__dirname, '../data/phase33f_retry');
...
const runId = `phase33f-retry-${Date.now()}`;
...
console.log("Starting Phase 33F RETRY - Real 30-Minute Validation...");
...
const watchdog = setInterval(() => {
    ...
    if (gap > 30000) {
        console.error(`[${new Date().toISOString()}] CRITICAL: Feed stale for > 30s. Marking test as FAILED.`);
        feedFailed = true;
        finishTest();
    } else if (gap > 5000) {
        staleEvents++;
        console.log(`[${new Date().toISOString()}] Feed STALE for ${gap}ms. Reconnecting...`);
        lastMessageTime = now; // reset temporarily to avoid spamming reconnects
        socket.disconnect();
        setTimeout(() => {
            socket.connect();
        }, 500);
...
socket.onevent = function(packet) {
    ...
    lastMessageTime = Date.now(); // Mark valid message received
...
function finishTest() {
    ...
    const summary = {
        ...
        duplicateOrderbookStates: validUpdates - snapshotHashes.size,
        signalsGenerated,
        signalsRejected,
    ...
    const metadata = {
        ...
        exchange: 'CoinDCX',
        symbol: 'B-SOL_USDT',
        strategyVersion: 'Phase 33F-RETRY',
    ...
    console.log("Phase 33F RETRY data collection finished.");
    process.exit(feedFailed ? 1 : 0);
}
