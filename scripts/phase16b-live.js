...
async function run() {
    console.log("Discovering symbols for Phase 16B (60 min run)...");
    let activeInstruments = [];
    try {
        activeInstruments = JSON.parse(fs.readFileSync(path.join(__dirname, '../research/phase13/phase13-futures-universe.json')));
    } catch(e) {
        console.error("Failed to load Phase 13 universe", e);
        return;
    }
    ...
    // Sort by lowest notional, pick 5
    candidates.sort((a,b) => a.min_notional - b.min_notional);
    const testPairs = candidates.slice(0, 5);

    console.log(`Selected pairs: ${testPairs.map(p=>p.pair).join(', ')}`);
    fs.writeFileSync(path.join(OUT_DIR, 'phase16b_symbols.json'), JSON.stringify(testPairs, null, 2));
    ...
    fs.writeFileSync(path.join(OUT_DIR, 'phase16b_orderbook.csv'), obCsv);
    fs.writeFileSync(path.join(OUT_DIR, 'phase16b_trades.csv'), trCsv);
    console.log("Phase 16B live data saved.");
}
run();
