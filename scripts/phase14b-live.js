...
async function run() {
    ...
    console.log(`Starting 60-minute live polling for ${testPairs.map(t=>t.pair).join(', ')}...`);
            ...
            try {
                ...
                if (obData && obData.bids && obData.asks) {
                    ...
                    if (bids.length > 0 && asks.length > 0) {
                        ...
                        const imbalance1 = bidQty1 / (bidQty1 + askQty1);
                        const imbalance5 = bidQty5 / (bidQty5 + askQty5);
                        
                        obRecord = { ...obRecord, bestBid: bids[0].price, bestAsk: asks[0].price, mid: (bids[0].price+asks[0].price)/2, spread, spreadPct, bidQty1, askQty1, bidQty5, askQty5, imbalance1, imbalance5 };
    ...
    console.log("Saving 60-minute data to CSV...");
    
    let obCsv = "symbol,timestamp,bestBid,bestAsk,mid,spread,spreadPct,bidQty1,askQty1,bidQty5,askQty5,imbalance1,imbalance5\n";
    ...
    for (const p of testPairs) {
        ...
        for (const ob of data[p.pair].orderbook) {
            obCsv += `${p.pair},${ob.timestamp},${ob.bestBid},${ob.bestAsk},${ob.mid},${ob.spread},${ob.spreadPct},${ob.bidQty1},${ob.askQty1},${ob.bidQty5},${ob.askQty5},${ob.imbalance1},${ob.imbalance5}\n`;
        }
        for (const tr of data[p.pair].trades) {
            trCsv += `${p.pair},${tr.timestamp},${tr.price},${tr.quantity},${tr.is_maker}\n`;
        }
    }
    fs.writeFileSync(path.join(OUT_DIR, 'phase14b-orderbook.csv'), obCsv);
    fs.writeFileSync(path.join(OUT_DIR, 'phase14b-trades.csv'), trCsv);
