...
const OUTPUT_FILE = path.join(__dirname, '../public/data/btc-inr-14d-1m.json');
const TARGET_CANDLES = 20160; // 14 days of 1-minute candles
...
async function run() {
  console.log(`Starting massive data fetch (Target: ${TARGET_CANDLES} candles)`);
  ...
  while (allCandles.length < TARGET_CANDLES) {
    try {
      console.log(`Fetching batch... (Current total: ${allCandles.length})`);
